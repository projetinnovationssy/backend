        ...
        try {
            ...
            if (range == null) {
                return ResponseEntity.status(HttpStatus.OK)
                        ...
                        .body(readByteRange(v.getVideo_location(), rangeStart, fileSize - 1)); // Read the object and convert it as bytes
            }
        ...
        try {
            ...
            if (range == null) {
                return ResponseEntity.status(HttpStatus.OK)
                        ...
                        .header(CONTENT_LENGTH, String.valueOf(fileSize))
                        .body(readByteRange(v.getThumbnail_location(), rangeStart, fileSize - 1)); // Read the object and convert it as bytes
            }
            String[] ranges = range.split("-");
            rangeStart = Long.parseLong(ranges[0].substring(6));
            if (ranges.length > 1) {
